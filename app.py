...
# ✅ Helper to safely extract text from Gemini responses
def get_model_text(response):
    """Extract plain text safely from Gemini responses."""
    if not response:
        return ""
    try:
...
def index():
    session.clear()
    # ✅ Render your wow.html page instead of index.html
...
def chat():
    ...
    try:
        ...
        extraction_text = get_model_text(extraction_response)
        match = re.search(r"\{[\s\S]*\}", extraction_text.strip())
        if match:
    ...
    try:
        ...
        ai_text = get_model_text(response).strip()
    except Exception as e:
        print("⚠️ Chat error:", e)
        ai_text = ""

    if not ai_text:
        ai_text = "I'm having trouble connecting to the chatbot right now."

    ai_reply = f"{forecast_text}\n{ai_text}".strip() if forecast_text else ai_text
